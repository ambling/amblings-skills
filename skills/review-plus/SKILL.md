# review-plus: Enhanced Code Review with QA Impact Detection

Extends the base `/review` skill to:
1. Detect QA test case impact during code review
2. Map changed files to affected test plans
3. Classify impact (BREAKING, NEEDS_UPDATE, NEW_TEST_NEEDED)
4. Propose specific updates to test steps and selectors
5. Integrate with `/qa-plus --sync` workflow

---

## Usage

### Basic Review (original behavior)
```bash
/review
```
Runs standard pre-landing PR review like `/review`.

### Review with QA Impact Detection
```bash
/review
```
...
## Integration with Fix-First Review Flow

QA issues are integrated into the Fix-First review workflow:
...
| Tool | Purpose |
|------|---------|
| `/qa-plus --sync` | Full sync workflow for test cases |
...
Inherits all `/review` parameters. No additional parameters required.
...
**Current:** 1.0.0
**Based on:** `/review` skill from gstack
